...
def has_repeating_pattern(num_str, length):
    # try all possible pattern lengths
    for size in range(1, length // 2 + 1):
        if length % size == 0:  # size must divide length
            pattern = num_str[:size]
            repeats = length // size
            if pattern * repeats == num_str:
#                print(f"Invalid ID found: {num_str} (pattern:{pattern}, repeats: {repeats})")  
                return True, pattern, repeats
    return False, None, 1

def total_invalid_ids(first, last):
    total_invalid_sum = 0
    for number in range(first, last + 1):
        str_num = str(number)
        length = len(str_num)
        is_repeating, pattern, repeats = has_repeating_pattern(str_num, length)
        if is_repeating:
...
def solve(input_text):
    ...
    for item in items:
        ...
        total_invalid_sum += total_invalid_ids(first, last)
    return total_invalid_sum
...
print(f"Total Invalid IDs sum: {result}")
